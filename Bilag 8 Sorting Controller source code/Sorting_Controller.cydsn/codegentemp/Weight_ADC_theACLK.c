/*******************************************************************************
* File Name: Weight_ADC_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_Start
********************************************************************************
*
* Summary:
...
void Weight_ADC_theACLK_Start(void) 
{
    ...
    Weight_ADC_theACLK_CLKEN |= Weight_ADC_theACLK_CLKEN_MASK;
	Weight_ADC_theACLK_CLKSTBY |= Weight_ADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
void Weight_ADC_theACLK_Stop(void) 
{
    ...
    Weight_ADC_theACLK_CLKEN &= (uint8)(~Weight_ADC_theACLK_CLKEN_MASK);
	Weight_ADC_theACLK_CLKSTBY &= (uint8)(~Weight_ADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Weight_ADC_theACLK_CLKEN & Weight_ADC_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Weight_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Weight_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Weight_ADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Weight_ADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Weight_ADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Weight_ADC_theACLK_GetSourceRegister();
    uint16 oldDivider = Weight_ADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Weight_ADC_theACLK_CLKEN & Weight_ADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Weight_ADC_theACLK_DIV_PTR, clkDivider);
                Weight_ADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Weight_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Weight_ADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Weight_ADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Weight_ADC_theACLK_DIV_PTR, clkDivider);
				Weight_ADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Weight_ADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Weight_ADC_theACLK_MOD_SRC |= modeBitMask & (uint8)Weight_ADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Weight_ADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Weight_ADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Weight_ADC_theACLK_MOD_SRC & (uint8)(Weight_ADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Weight_ADC_theACLK_GetDividerRegister();
    uint8 oldSrc = Weight_ADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Weight_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
        Weight_ADC_theACLK_MOD_SRC =
            (Weight_ADC_theACLK_MOD_SRC & (uint8)(~Weight_ADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Weight_ADC_theACLK_MOD_SRC & Weight_ADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Weight_ADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Weight_ADC_theACLK_PHASE = clkPhase & Weight_ADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Weight_ADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Weight_ADC_theACLK_PHASE & Weight_ADC_theACLK_PHASE_MASK;
}
