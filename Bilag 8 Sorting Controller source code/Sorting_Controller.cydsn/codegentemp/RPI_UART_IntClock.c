...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
void RPI_UART_IntClock_Start(void) 
{
    ...
    RPI_UART_IntClock_CLKEN |= RPI_UART_IntClock_CLKEN_MASK;
	RPI_UART_IntClock_CLKSTBY |= RPI_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void RPI_UART_IntClock_Stop(void) 
{
    ...
    RPI_UART_IntClock_CLKEN &= (uint8)(~RPI_UART_IntClock_CLKEN_MASK);
	RPI_UART_IntClock_CLKSTBY &= (uint8)(~RPI_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((RPI_UART_IntClock_CLKEN & RPI_UART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = RPI_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = RPI_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* RPI_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(RPI_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(RPI_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = RPI_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = RPI_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = RPI_UART_IntClock_CLKEN & RPI_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(RPI_UART_IntClock_DIV_PTR, clkDivider);
                RPI_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                RPI_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(RPI_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((RPI_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(RPI_UART_IntClock_DIV_PTR, clkDivider);
				RPI_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(RPI_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    RPI_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)RPI_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    RPI_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(RPI_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return RPI_UART_IntClock_MOD_SRC & (uint8)(RPI_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = RPI_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = RPI_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        RPI_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        RPI_UART_IntClock_MOD_SRC =
            (RPI_UART_IntClock_MOD_SRC & (uint8)(~RPI_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return RPI_UART_IntClock_MOD_SRC & RPI_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: RPI_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    RPI_UART_IntClock_PHASE = clkPhase & RPI_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: RPI_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return RPI_UART_IntClock_PHASE & RPI_UART_IntClock_PHASE_MASK;
}
